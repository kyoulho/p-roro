...
class DiscoveredThirdPartyControllerTest {
    ...
    @Test
    void discovered_서드파티목록조회() throws Exception {
        String serviceIds = "1,2,3";

        mockMvc.perform(get("/api/projects/{projectId}/third-parties", 1)
    ...
    @Test
    void discovered_서버서드파티목록조회() throws Exception {
        mockMvc.perform(get("/api/projects/{projectId}/inventory/servers/{serverId}/third-parties", 1, 2)
                        .header(HttpHeaders.AUTHORIZATION, accessToken)
                        .contentType(MediaType.APPLICATION_JSON_VALUE))
                .andDo(print())
                .andExpect(status().isOk())
