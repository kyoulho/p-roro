 ...
 * @version 1.0
 */
public class NginxExecutePathScenario extends ExtractScenario {

    public NginxExecutePathScenario(TargetHost targetHost) {
    ...
    protected boolean extract(Process process, CommandConfig commandConfig, GetInfoStrategy strategy) throws InterruptedException {
        String executePath = process.getCmd().get(3);

        if (StringUtils.defaultString(executePath).equals("nginx")) {
            result = MWCommonUtil.getExecuteResult(targetHost, COMMAND.COMMAND_WHICH, commandConfig, strategy, "nginx");
        } else {
            result = executePath;
        }
...
//end of NginxParentDirScenario.java