 ...
 * @version 1.0
 */
public class WebSphereProfileScenario extends ExtractScenario {
    @Override
    protected boolean extract(Process process, CommandConfig commandConfig, GetInfoStrategy strategy) {
        if (StringUtils.isNotEmpty(ProcessCmdUtil.getParam(process.getCmd(), "-Dserver.root"))) {
            result = ProcessCmdUtil.getParam(process.getCmd(), "-Dserver.root");
        } else if (StringUtils.isNotEmpty(ProcessCmdUtil.getParam(process.getCmd(), "-Duser.install.root"))) {
            result = ProcessCmdUtil.getParam(process.getCmd(), "-Duser.install.root");
        }
...
//end of WebSphereProfileScenario.java